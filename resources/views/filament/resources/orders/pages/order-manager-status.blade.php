...
<section x-data="{ activeTab: 'info' }" class="order-status-panel relative min-h-0 min-w-0 overflow-y-auto">
    {{-- Lớp loading chỉ phủ panel phải, bảng Order bên trái vẫn giữ nguyên và tiếp tục tương tác. --}}
    <div wire:loading.flex
         class="absolute inset-0 z-50 hidden items-center justify-center rounded-2xl bg-white/65 backdrop-blur-[1px]">
        <div
            class="oms-card flex items-center gap-3 px-4 py-3 font-semibold text-slate-600">
    ...
    <div class="oms-card mb-4 p-6">
        <div class="flex items-start justify-between gap-4">
            ...
            <div class="flex gap-2 text-xl">
                <button
                    ...
                    <x-heroicon-o-arrow-path wire:loading.class="animate-spin" wire:target="duplicateSelectedOrder"
                                             class="h-5 w-5"/>
                </button>
        ...
        <div class="mt-8 grid gap-3 border-t border-slate-300 pt-5 sm:grid-cols-2">
            <label
                class="oms-card flex cursor-pointer items-center gap-3 bg-slate-50 px-4 py-3 font-bold text-slate-700 transition hover:border-emerald-300">
                <input type="checkbox" wire:model.live="isDelivered"
                       class="h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500">
                <span>Đã giao</span>
            </label>
            <label
                class="oms-card flex cursor-pointer items-center gap-3 bg-slate-50 px-4 py-3 font-bold text-slate-700 transition hover:border-blue-300">
                <input type="checkbox" wire:model.live="isPaid"
    ...
    <div
        class="oms-tabs mb-4 text-lg font-bold">
        <button @click="activeTab = 'info'" :aria-selected="activeTab === 'info'"
                :class="activeTab === 'info' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-slate-500'"
                class="oms-tab">Thông tin
        </button>
        <button @click="activeTab = 'products'" :aria-selected="activeTab === 'products'"
                :class="activeTab === 'products' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-slate-500'"
                class="oms-tab">Sản phẩm
        </button>
        <button @click="activeTab = 'design'" :aria-selected="activeTab === 'design'"
                :class="activeTab === 'design' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-slate-500'"
                class="oms-tab">Thiết kế (2)
        </button>
        <button @click="activeTab = 'history'" :aria-selected="activeTab === 'history'"
                :class="activeTab === 'history' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-slate-500'"
                class="oms-tab">Lịch sử
        </button>
        <button @click="activeTab = 'payment'" :aria-selected="activeTab === 'payment'"
                :class="activeTab === 'payment' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-slate-500'"
                class="oms-tab">Thanh toán
        </button>
        <button @click="activeTab = 'note'" :aria-selected="activeTab === 'note'"
                :class="activeTab === 'note' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-slate-500'"
                class="oms-tab">Ghi chú
        </button>
    </div>
    {{-- Wrapper duy nhất của vùng nội dung; các tab chỉ thay đổi phần tử con bên trong wrapper này. --}}
    <div id="contentTab">
        <div x-show="activeTab === 'info'" class="mb-4 grid gap-4 lg:grid-cols-2">
            <div class="oms-card p-5">
            ...
            <div class="oms-card p-5">
                ...
                <div class="space-y-4 text-sm">
                    <p class="flex justify-between"><span class="text-slate-500">Mã đơn</span><b>{{ $detailCode }}</b>
                    </p>
                    <p class="flex justify-between"><span
                            class="text-slate-500">Ngày tạo</span><b>{{ $detailCreatedAt }}</b></p>
                    <p class="flex justify-between"><span
                            class="text-slate-500">Trạng thái</span><b>{{ $detailStatusLabel }}</b></p>
                    <p class="flex justify-between"><span
                            class="text-slate-500">Tạm tính</span><b>{{ $detailSubtotal }}</b></p>
                    <p class="flex justify-between"><span
                            class="text-slate-500">Giảm giá</span><b>{{ $detailDiscount }}</b></p>
                    <p class="flex justify-between"><span
                            class="text-slate-500">Tổng tiền</span><b>{{ $detailTotal }}</b></p>
                </div>
            </div>
        </div>
        {{-- Các tab còn lại dùng nội dung mẫu vì chưa có nghiệp vụ chi tiết tương ứng. --}}
        <div x-show="activeTab === 'products'" class="oms-card mb-4 p-6">
            ...
            <div class="mt-4 overflow-x-auto rounded-xl border border-slate-300">
                <table class="w-full min-w-[620px] text-left text-sm">
                    ...
                    @forelse ($productRows as $itemIndex => $item)
                        <tr>
                            <td class="px-4 py-4">{{ $itemIndex + 1 }}</td>
                            ...
                            <td class="px-4 py-4">{{ $item['unitPrice'] }}</td>
                            <td class="px-4 py-4 font-bold">{{ $item['subtotal'] }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-6 text-center text-slate-500">Chưa có order item, đang hiển
                                thị nội dung mẫu.
                            </td>
                        </tr>
                    @endforelse
        ...
        <div x-show="activeTab === 'design'" class="oms-card mb-4 p-6">
        ...
        <div x-show="activeTab === 'history'" class="oms-card mb-4 p-6">
            ...
            <div class="mt-4 space-y-3 text-sm"><p class="rounded-xl bg-slate-50 p-4">Đang in<span
                ...
                <p class="rounded-xl bg-slate-50 p-4">Đang thiết kế<span
                        class="ml-3 text-slate-500">03/09/2026 15:10</span>
                </p></div>
        </div>
        <div x-show="activeTab === 'payment'" class="oms-card mb-4 p-6">
        ...
        <div x-show="activeTab === 'note'" class="oms-card mb-4 p-6">
            <h3 class="text-lg font-extrabold">Ghi chú đơn hàng</h3>
            <div
                class="mt-4 whitespace-pre-line rounded-xl bg-amber-50 p-4 text-sm leading-7 text-slate-600">{{ $detailNote }}</div>
        </div>
    </div>
    <div class="relative grid gap-4 lg:grid-cols-2 mb-4">
        <div class="oms-card p-5">
            ...
            <div
                class="whitespace-pre-line rounded-xl bg-amber-50 p-4 text-sm leading-7 text-slate-600">{{ $detailNote }}</div>
        </div>
        <div class="oms-card p-5"><h3 class="text-lg font-extrabold">Tổng
                tiền</h3>
    ...
    <div id="button" class="oms-card space-y-4 p-5">
        {{-- Card riêng gom ghi chú, tổng tiền và nhóm thao tác cuối đơn hàng. --}}

            <div class="flex flex-wrap gap-3 rounded-2xl p-4">
