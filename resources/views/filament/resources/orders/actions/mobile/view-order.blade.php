<div x-data="{ copyFeedback: '' }" class="flex h-full flex-col bg-white">
    <header class="shrink-0 border-b border-slate-100 px-4 py-3">
        <h1 class="text-[22px] font-bold leading-7 tracking-[-0.3px]">Chi tiết đơn hàng</h1>
    </header>
    ...
    <div class="min-h-0 flex-1 overflow-y-auto overscroll-contain">
        <section class="space-y-4 p-4">
            <div>
                <div class="flex flex-wrap items-center gap-2">
                    <h2 class="break-all text-[22px] font-bold leading-7 tracking-[-0.4px]">#{{ $order->order_code }}</h2>
                    <span class="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-[13px] font-semibold leading-5 {{ $currentStatus['badge_classes'] }}">
                        <span class="size-1.5 rounded-full bg-current opacity-70"></span>
                        {{ $currentStatus['label'] }}
                    </span>
                </div>
                <div class="mt-2 space-y-1 text-[13px] leading-5 text-slate-500">
                    <p>Tạo ngày {{ $order->order_date?->format('d/m/Y H:i') ?? '--/--' }}</p>
                    <p>Cập nhật {{ $order->updated_at?->format('d/m/Y H:i') ?? '--/--' }}</p>
                </div>
            </div>

            <section class="rounded-xl border border-blue-100 bg-blue-50/50 p-3.5" aria-label="Tiến trình xử lý">
                <h3 class="flex items-center gap-2 text-[15px] font-semibold leading-[22px]">
                ...
                <div class="relative mt-4">
                    ...
                    <ol class="relative grid grid-cols-3 gap-1 text-center text-[13px] leading-5">
            ...
            <div class="grid grid-cols-2 gap-2.5">
                <section class="flex min-w-0 items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50/60 p-3">
                    ...
                    <div class="min-w-0">
                        <h3 class="text-[15px] font-semibold leading-5">Giao hàng</h3>
                        <p class="mt-1 truncate text-[13px] leading-5 text-slate-500">{{ $order->is_delivered ? 'Đã giao' : 'Chưa giao' }}</p>
                    </div>
                </section>
                <section class="flex min-w-0 items-center gap-2 rounded-xl border border-blue-200 bg-blue-50/60 p-3">
                    ...
                    <div class="min-w-0">
                        <h3 class="text-[15px] font-semibold leading-5">Thanh toán</h3>
                        <p class="mt-1 truncate text-[13px] leading-5 text-slate-500">{{ $order->is_paid ? 'Đã thanh toán' : 'Chưa thanh toán' }}</p>
                    </div>
                </section>
            </div>
        </section>
        ...
        <nav class="sticky top-0 z-10 border-b border-slate-200 bg-white" aria-label="Thông tin đơn hàng">
            <div role="tablist" aria-label="Chi tiết" class="flex overflow-x-auto px-4">
                @foreach ($tabs as $tabKey => $tab)
                    <button
                        ...
                        class="shrink-0 whitespace-nowrap border-b-2 px-3 py-3.5 text-[15px] font-medium leading-[22px]"
                    >
                        {{ $tab['label'] }}
                        @isset($tab['count'])
                            <span class="ml-1 text-[13px] leading-5 text-slate-400">({{ $tab['count'] }})</span>
                        @endisset
                    </button>
                @endforeach
            </div>
                            ...
                            @forelse ($order->items as $item)
                                ...
                                <article class="rounded-xl border border-slate-200 bg-white p-3.5">
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div class="min-w-0">
                                            <h4 class="break-words text-[17px] font-semibold leading-6 tracking-[-0.15px]">{{ $item->productSku?->product?->name ?? 'Sản phẩm' }}</h4>
                                            <p class="mt-1 break-all text-[13px] leading-5 text-slate-500">SKU: {{ $item->productSku?->sku_code ?? '---' }}</p>
                                            <p class="mt-1 text-[13px] leading-5 text-slate-400">{{ $item->productSku?->product?->note ?? 'Chưa có mô tả' }}</p>
                                        </div>
                                    </div>
                                    <dl class="mt-4 grid grid-cols-2 border-y border-slate-100 py-3">
                                        <div class="border-r border-slate-100">
                                            <dt class="text-[13px] leading-5 text-slate-500">Số lượng</dt>
                                            <dd class="mt-1 text-[17px] font-semibold leading-6 tabular-nums">{{ number_format($item->quantity, 0, ',', '.') }}</dd>
                                        </div>
                                        <div class="pl-4">
                                            <dt class="text-[13px] leading-5 text-slate-500">Đơn giá</dt>
                                            <dd class="mt-1 text-[16px] leading-6 tabular-nums">{{ number_format((float) $item->unit_price, 0, ',', '.') }}đ</dd>
                                        </div>
                                    </dl>
                                    <div class="flex items-center justify-between gap-2 py-3 text-[15px] leading-[22px]">
                                        <span class="text-slate-500">Tiền sản phẩm</span>
                                        <strong class="text-[17px] font-semibold leading-6 tabular-nums">{{ number_format((float) $item->subtotal, 0, ',', '.') }}đ</strong>
                                    </div>
                                    <div class="space-y-2">
                                        @forelse ($item->services as $service)
                                            <div class="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-blue-50 p-3">
                                                <div class="flex min-w-0 items-center gap-2">
                                                    ...
                                                    <div class="min-w-0">
                                                        <h5 class="truncate text-[15px] font-medium leading-5 text-blue-600">{{ $service->service_name }}</h5>
                                                        <p class="mt-1 text-[13px] leading-5 text-slate-500">{{ number_format($service->quantity, 0, ',', '.') }} × {{ number_format((float) $service->unit_price, 0, ',', '.') }}đ</p>
                                                    </div>
                                                </div>
                                                <strong class="ml-auto text-[17px] font-semibold leading-6 tabular-nums">{{ number_format((float) $service->subtotal, 0, ',', '.') }}đ</strong>
                                            </div>
                                        @empty
                                            <p class="rounded-lg bg-slate-50 p-3 text-[13px] leading-5 text-slate-400">Dịch vụ: Không có</p>
                                        @endforelse
                                    </div>
                                    <div class="mt-3 flex items-center justify-between gap-2 border-t border-slate-100 pt-3">
                                        <span class="text-[15px] font-semibold text-slate-700">Thành tiền</span>
                                        <strong class="text-[18px] font-bold text-blue-600 tabular-nums">{{ number_format($itemTotal, 0, ',', '.') }}đ</strong>
                                    </div>
                                </article>
                            @empty
