            ...
            <div x-show="tab === 'inventory'" x-cloak>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full min-w-[1100px] border-collapse">
                            ...
                            <tr class="bg-slate-100/90 text-left">
                                <th class="px-4 py-3 text-left font-semibold text-gray-700 dark:text-gray-200">Sản
                                    phẩm
                                </th>
                                ...
                                <th class="px-4 py-3 text-right font-semibold text-gray-700 dark:text-gray-200">Đã
                                    xuất
                                </th>
                                <th class="px-4 py-3 text-right font-semibold text-gray-700 dark:text-gray-200">Còn
                                    lại
                                </th>
                                <th class="px-4 py-3 text-left font-semibold text-gray-700 dark:text-gray-200">Dịch vụ
                                </th>
                                <th class="px-4 py-3 text-right font-semibold text-gray-700 dark:text-gray-200">Phí DV
                                    còn lại
                                </th>
                                ...
                                <tr wire:key="release-stock-item-{{ $item->id }}" class="transition hover:bg-slate-50">
                                    ...
                                    <td>
                                        @forelse ($item->orderItem->services as $service)
                                            ...
                                            <p class="text-xs text-slate-500">{{ number_format((float) $service->unit_price, 0, ',', '.') }}
                                                đ/SP</p>
                                        @empty
                                    ...
                                    <td class="text-right font-medium text-blue-700">
                                        {{ number_format($remainingQuantity * $item->orderItem->services->sum(fn ($service): float => (float) $service->unit_price), 0, ',', '.') }}
                                        đ
                                    </td>
                                    <td>
                                        <input
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="py-8 text-center text-slate-500">Order này chưa có hàng
                                        tồn.
                                    </td>
                                </tr>
                            @endforelse
                ...
                <div class="mt-3 grid gap-3 rounded-xl border border-blue-100 bg-blue-50/60 p-4 text-sm sm:grid-cols-3">
                    <div>
                        ...
                        <p class="mt-1 font-bold text-slate-900">{{ number_format($releaseProductPreview, 0, ',', '.') }}
                            đ</p>
                    </div>
                    <div>
                        <p class="text-slate-500">Tiền dịch vụ dự kiến</p>
                        <p class="mt-1 font-bold text-blue-700">{{ number_format($releaseServicePreview, 0, ',', '.') }}
                            đ</p>
                    </div>
                    <div>
                        <p class="text-slate-500">Tạm tính trước phân bổ</p>
                        <p class="mt-1 font-bold text-slate-900">{{ number_format($releaseProductPreview + $releaseServicePreview, 0, ',', '.') }}
                            đ</p>
                    </div>
                </div>
            </div>
                        ...
                        <table class="w-full min-w-[1200px] border-collapse">
                            ...
                            @forelse ($customerStock->releases as $release)
                                <tr wire:key="release-history-{{ $release->id }}" class="transition hover:bg-slate-50">
                                    ...
                                    <td class="text-right">{{ number_format((float) $release->gross_product_amount, 0, ',', '.') }}
                                        đ
                                    </td>
                                    <td class="text-right font-medium text-blue-700">{{ number_format((float) $release->gross_service_amount, 0, ',', '.') }}
                                        đ
                                    </td>
                                    <td class="text-right text-red-600">
                                        -{{ number_format((float) $release->allocated_discount, 0, ',', '.') }}đ
                                    </td>
                                    <td class="text-right text-slate-500">
                                        {{ (float) $release->reconciliation_adjustment > 0 ? '+' : '' }}{{ number_format((float) $release->reconciliation_adjustment, 0, ',', '.') }}
                                        đ
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="12" class="py-8 text-center text-slate-500">Chưa có phiếu xuất hàng.
                                    </td>
                                </tr>
                            @endforelse
                            </tbody>
